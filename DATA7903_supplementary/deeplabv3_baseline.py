...
if device == "cuda":
    print("Using Cuda")
print("deeplabv3_fixed_before_update")
...
# random.seed(42)
# all_images = random.sample(all_images, 50)

kf = KFold(n_splits=5, shuffle=True, random_state=42) # was 2
...
def plot_confusion_matrix(cm, labels):
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
                xticklabels=[l["id"] for l in labels],
                yticklabels=[l["id"] for l in labels])
    plt.xlabel("Predicted")
    plt.ylabel("Ground Truth")
    plt.title("Pixel-wise Confusion Matrix")
    plt.savefig("deeplabv3_baseline_confusion.png")
    # plt.show()
...
def compute_iou_per_class(cm):
    """
    Compute IoU for each class and mean IoU from confusion matrix.
    """
    ious = []
